...
router.get('/problems/tests/:problem', [
...
router.post('/problems/addtest/:problem', [
    check('problem').isString().not().isEmpty()
], async (req, res) => {
    const errors = validationResult(req)
    if (!errors.isEmpty()) {
    ...
    tests.createTest(req.params.problem, stream).then(r => {
        res.sendStatus(200)
    }).catch(err => {
        console.log(err)
...
router.post('/submit', [
    ...
    const lang = await Lang.findOne({ name: req.body.lang }).catch(err => {
            console.log(err)
            res.sendStatus(500)
    })
    if (!lang)
