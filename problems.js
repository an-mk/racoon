...
async function insertProblem(name, content, checkEnv, checkCodeStream) {
    ...
    if (envInstance.usesBinary) {
        ...
        await new Promise(resolve => {
            ...
            if (checkCodeStream instanceof Readable)
                checkCodeStream.pipe(writeStream)
            else
                writeStream.end(checkCodeStream)
        })
        ...
        try {
            ...
            if (err[0] > 1) {
                ...
                console.error('Compilation failed:', err[1])
                return [false, err[1]]
            }
        }
    ...
    if (isFound) await Problem.deleteOne({ name: name })
        ...
        .catch((err) => {
            console.error('Failed to override Problem:', err)
            throw err
        });
    return await problem.save()
        ...
        .catch((err) => {
            console.error('Failed to add Problem to the database:', err)
            throw err
        })
}
